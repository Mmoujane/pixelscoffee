...
export async function POST(req) {
  ...
  try {
    ...
    const Path = path.join(process.cwd(), 'public', 'media', 'blogs', fileName);
    ...
    const newBlog = {
      title,
      content, // Ensure price is a number
      image: `/media/blogs/${fileName}`,
      createdAt: new Date(),
      updatedAt: new Date(),
    };
    ...
    return NextResponse.json(JSON.stringify({ message: 'blog added successfully', productId: result.insertedId }), {status :201});
  } catch (error) {
